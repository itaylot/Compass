...
export function InfoScreen(props: {
  ...
  const { settings, items, reminders, currentDay, focusOnMap, saveSettings, savingSettings, chooseLodging, showToast, lastUpdatedLabel } =
    props;
  ...
  const shareApp = async () => {
    ...
    try {
      if (navigator.share) {
        await navigator.share({ title: settings.title, url: link });
        return;
      }
  ...
  return (
    <div className="flex flex-col gap-3 pb-4">
            ...
            <div key={group} className="rounded-xl bg-[var(--paper)] p-2.5">
              ...
              <div className="flex flex-col gap-1.5">
                {options.map((h) => {
                  ...
                  return (
                    <div key={h.id} className={`rounded-lg bg-white px-3 py-2 ${chosen ? "ring-2 ring-[var(--brand)]" : ""}`}>
                      ...
                      {h.shortDescription && <p className="mt-0.5 text-[11px] text-[var(--muted)]">{h.shortDescription}</p>}
                    </div>
                  );
                })}
              </div>
      ...
      {/* שיתוף */}
      <Section title="שיתוף האפליקציה" icon="🔗">
        <p className="mb-2 text-xs text-[var(--muted)]">
          שלחו את הקישור לכל המשפחה. לכולם יש אותה גישה — כולם רואים ויכולים לערוך.
        </p>
        <button onClick={shareApp} className="w-full rounded-full bg-[var(--brand)] px-4 py-2.5 text-sm font-bold text-white">
          שיתוף / העתקת הקישור 📤
        </button>
      </Section>
